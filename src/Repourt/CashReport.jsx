    ...
    <div style={{ padding: '15px', direction: 'rtl', fontFamily: "'Tajawal', sans-serif" }}>
      ...
      <div className="glass-card" style={{ borderRight: '6px solid #e67e22', textAlign: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '15px' }}>
          <div style={{ background: 'rgba(240, 253, 244, 0.8)', padding: '12px', borderRadius: '14px' }}>
            <TrendingUp size={16} color="#27ae60" />
            <div style={{ fontSize: '0.8rem', color: '#64748b' }}>إجمالي الوارد</div>
            <strong style={{ color: '#27ae60' }}>{totalIn.toLocaleString()}</strong>
          </div>
          <div style={{ background: 'rgba(254, 242, 242, 0.8)', padding: '12px', borderRadius: '14px' }}>
            <TrendingDown size={16} color="#e74c3c" />
            <div style={{ fontSize: '0.8rem', color: '#64748b' }}>إجمالي المنصرف</div>
            <strong style={{ color: '#e74c3c' }}>{totalOut.toLocaleString()}</strong>
          </div>
        </div>

        <div style={{ marginTop: '15px', padding: '12px', background: 'rgba(248, 250, 252, 0.8)', borderRadius: '14px' }}>
          <span style={{ fontSize: '0.85rem', color: '#64748b' }}>رصيد الخزينة:</span>
          <div style={{ fontSize: '1.5rem', fontWeight: '800', color: (totalIn - totalOut) >= 0 ? '#27ae60' : '#e74c3c' }}>
            {(totalIn - totalOut).toLocaleString()} ج.م
          </div>
        </div>

        <button onClick={exportCashFlowExcel} className="btn-primary" style={{ backgroundColor: '#e67e22', boxShadow: '0 4px 15px rgba(230, 126, 34, 0.3)', marginTop: '20px' }}>
          <Download size={20} /> تحميل تقرير الخزينة (Excel)
        </button>
      </div>
    </div>
  );
