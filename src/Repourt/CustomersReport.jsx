  ...
  return (
    <div style={{ padding: '15px', direction: 'rtl', fontFamily: "'Tajawal', sans-serif" }}>
      ...
      <div className="glass-card" style={{ borderRight: '6px solid #27ae60', textAlign: 'center' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '0.9rem' }}>تقرير مفصل يوضح أكثر العملاء شراءً وحجم التعاملات معهم.</p>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '15px' }}>
          <div style={{ background: 'rgba(240, 253, 244, 0.8)', padding: '12px', borderRadius: '14px' }}>
            <div style={{ fontSize: '0.8rem', color: '#64748b' }}>عدد العملاء</div>
            <strong style={{ color: '#27ae60', fontSize: '1.2rem' }}>{uniqueCustomers.length}</strong>
          </div>
          <div style={{ background: 'rgba(240, 253, 244, 0.8)', padding: '12px', borderRadius: '14px' }}>
            <div style={{ fontSize: '0.8rem', color: '#64748b' }}>إجمالي التعاملات</div>
            <strong style={{ color: '#27ae60', fontSize: '1.2rem' }}>{totalRevenue.toLocaleString()}</strong>
          </div>
        </div>

        <button onClick={exportCustomersExcel} className="btn-primary" style={{ backgroundColor: '#27ae60', boxShadow: '0 4px 15px rgba(39, 174, 96, 0.3)', marginTop: '20px' }}>
          <Download size={20} /> تحميل سجل العملاء (Excel)
        </button>
      </div>
    </div>
  );
