  ...
  return (
    <div style={{ padding: '15px', direction: 'rtl', fontFamily: "'Tajawal', sans-serif" }}>
      ...
      <div className="glass-card" style={{ borderRight: '6px solid #34495e', textAlign: 'center' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '0.9rem' }}>تحميل كافة فواتير المشتريات مجمعة حسب كل مورد وحالة مديونياته.</p>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '15px' }}>
          <div style={{ background: 'rgba(241, 245, 249, 0.8)', padding: '12px', borderRadius: '14px' }}>
            <div style={{ fontSize: '0.8rem', color: '#64748b' }}>عدد الموردين</div>
            <strong style={{ color: '#34495e', fontSize: '1.2rem' }}>{uniqueSuppliers.length}</strong>
          </div>
          <div style={{ background: 'rgba(241, 245, 249, 0.8)', padding: '12px', borderRadius: '14px' }}>
            <div style={{ fontSize: '0.8rem', color: '#64748b' }}>إجمالي التعاملات</div>
            <strong style={{ color: '#34495e', fontSize: '1.2rem' }}>{totalPurchases.toLocaleString()}</strong>
          </div>
        </div>

        <button onClick={exportSuppliersExcel} className="btn-primary" style={{ backgroundColor: '#34495e', boxShadow: '0 4px 15px rgba(52, 73, 94, 0.3)', marginTop: '20px' }}>
          <Download size={20} /> تحميل سجل الموردين (Excel)
        </button>
      </div>
    </div>
  );
