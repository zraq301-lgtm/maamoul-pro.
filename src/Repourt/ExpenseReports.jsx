...
export default function ExpenseReports({ expensesData = [], onBack }) {
  const exportExpensesExcel = () => {
    ...
    const data = expensesData.map(exp => ({
      "التاريخ": exp.date,
      "بند المصروف": exp.category || 'غير مصنف',
  ...
  return (
    <div style={{ padding: '15px', direction: 'rtl', fontFamily: "'Tajawal', sans-serif" }}>
      ...
      <div className="glass-card" style={{ borderRight: '6px solid #7f8c8d', textAlign: 'center' }}>
        ...
        <div style={{ background: 'rgba(248, 250, 252, 0.8)', padding: '15px', borderRadius: '15px', marginTop: '15px', border: '1px dashed rgba(203, 213, 225, 0.5)' }}>
        ...
        <button onClick={exportExpensesExcel} className="btn-primary" style={{ backgroundColor: '#34495e', boxShadow: '0 4px 15px rgba(52, 73, 94, 0.3)', marginTop: '20px' }}>
          <FileSpreadsheet size={20} /> تحميل كشف المصروفات (Excel)
        </button>
      </div>
      ...
      <div style={{ marginTop: '20px' }}>
        ...
        {expensesData.length === 0 ? (
          ...
          expensesData.slice(-5).reverse().map((exp, index) => (
            <div key={index} className="glass-card" style={{ marginBottom: '8px', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
