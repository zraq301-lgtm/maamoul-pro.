...
const App = () => {
  ...
  // 1. 📥 محرك جلب البيانات السحابي الآمن والمنظم (استخدام useCallback لمنع التكرار اللانهائي)
  const downloadDataFromMaamoulCloud = useCallback(async () => {
    try {
      ...
      setIsInitialLoading(false); // نضمن إيقاف التحميل حتى عند الفشل ليعتمد على التخزين المحلي
    }
  }, [setters]);
  ...
  const handleSavePurchase = async (p) => {
    const nextInventory = [...inventory, p];
    setInventory(nextInventory);
    saveLocally('inventory', nextInventory);
    ...
    setStock(prev => {
      ...
      nextStock = [...prev, { id: Date.now(), name: p.item, balance: parseFloat(p.quantity), price: p.price }];
      return nextStock;
    });

    // حفظ فوري للمخزون المحدث لمنع الفقدان المفاجئ
    setTimeout(() => {
      if (nextStock.length > 0) saveLocally('stock', nextStock);
    }, 100);
  };

  // حلقة حفظ ذكية منفصلة وموجهة للـ States المتبقية لمنع عمليات الكتابة العشوائية المتداخلة
  useEffect(() => {
