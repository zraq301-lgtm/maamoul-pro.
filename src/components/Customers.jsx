...
const Customers = ({ onBack, customers = [], onAddCustomer }) => {
  ...
  const handleAdd = () => {
    ...
    onAddCustomer({ ...newCust, id: Date.now() });
      ...
      {!showAdd ? (
        <>
          ...
          <button
            ...
            style={{ backgroundColor: '#27ae60', marginBottom: '15px', boxShadow: '0 4px 15px rgba(39, 174, 96, 0.3)' }}
          >
        ...
        <div className="glass-card">
          ...
          <input className="glass-input" placeholder="اسم العميل" value={newCust.name} onChange={e => setNewCust({ ...newCust, name: e.target.value })} style={{ marginBottom: '10px' }} />
          <label className="form-label"><Phone size={14} /> رقم الموبايل</label>
          <input className="glass-input" placeholder="01xxxxxxxxx" value={newCust.phone} onChange={e => setNewCust({ ...newCust, phone: e.target.value })} style={{ marginBottom: '10px' }} />
          <label className="form-label"><MapPin size={14} /> العنوان / المنطقة</label>
          <input className="glass-input" placeholder="مثال: المعادي" value={newCust.address} onChange={e => setNewCust({ ...newCust, address: e.target.value })} style={{ marginBottom: '15px' }} />
          <button onClick={handleAdd} className="btn-primary" style={{ backgroundColor: '#27ae60', marginBottom: '10px' }}>
