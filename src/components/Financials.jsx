  ...
  return (
    <div style={{ padding: '15px', direction: 'rtl', fontFamily: "'Tajawal', sans-serif", minHeight: '100vh' }}>
      <div className="page-header">
        <BarChart3 size={28} color="#16a085" />
        ...
        {statItems.map((item, i) => (
          <div key={i} className="glass-card" style={{ padding: '14px', background: item.bg }}>
            ...
            <div style={{ fontSize: '1.2rem', fontWeight: '800', color: item.color }}>
              {item.value.toLocaleString()} <small style={{ fontSize: '0.7rem' }}>ج.م</small>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="glass-card" style={{
        padding: '25px',
        textAlign: 'center',
        background: netProfit >= 0 ? 'rgba(220, 252, 231, 0.8)' : 'rgba(254, 226, 226, 0.8)',
        marginBottom: '15px'
      }}>
        <div style={{ fontSize: '0.95rem', color: '#475569', marginBottom: '8px' }}>صافي الأرباح</div>
        <div style={{ fontSize: '2rem', fontWeight: '900', color: netProfit >= 0 ? '#166534' : '#991b1b' }}>
          {netProfit.toLocaleString()} <small style={{ fontSize: '1rem' }}>ج.م</small>
        </div>
      </div>
        ...
        <div className="glass-card" style={{ marginBottom: '15px' }}>
          ...
          {cashBook.slice(-10).reverse().map((entry, i) => (
            <div key={i} style={{
              ...
              padding: '10px 0', borderBottom: i < 9 ? '1px solid rgba(226, 232, 240, 0.5)' : 'none'
            }}>
              <div>
                <div style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#1e293b' }}>{entry.description || entry.category}</div>
                <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{entry.timestamp}</div>
              </div>
              <div style={{
                fontWeight: '800',
                color: entry.type === 'in' ? '#2ecc71' : '#e74c3c',
                fontSize: '0.95rem'
              }}>
                {entry.type === 'in' ? '+' : '-'}{parseFloat(entry.amount || 0).toLocaleString()}
              </div>
            </div>
          ))}
        </div>
