      ...
      <div className="glass-card" style={{ border: '1px solid rgba(239, 68, 68, 0.2)' }}>
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
          <button
            ...
            style={{
              backgroundColor: wasteEntry.type === 'raw_material' ? '#e74c3c' : 'rgba(241, 245, 249, 0.8)',
              color: wasteEntry.type === 'raw_material' ? 'white' : '#475569',
          ...
          <button
            ...
            className="btn-primary"
            style={{
              backgroundColor: wasteEntry.type === 'product' ? '#e74c3c' : 'rgba(241, 245, 249, 0.8)',
              color: wasteEntry.type === 'product' ? 'white' : '#475569',
              boxShadow: 'none', fontSize: '0.9rem', padding: '12px'
            }}
          >
            منتج نهائي
        ...
        <form onSubmit={handleSubmit}>
          ...
          <select className="glass-input" value={wasteEntry.itemName} onChange={(e) => handleItemSelect(e.target.value)} style={{ marginBottom: '12px' }}>
            <option value="">اختر صنف من المخزن...</option>
            {inventory.map(s => (
              <option key={s.id} value={s.name}>{s.name} (رصيد: {s.balance})</option>
            ))}
          </select>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
            <div>
              <label className="form-label"><Layers size={16} color="#e74c3c" /> الكمية</label>
              <input type="number" className="glass-input" placeholder="0.00" value={wasteEntry.quantity} onChange={(e) => setWasteEntry({ ...wasteEntry, quantity: e.target.value })} />
            </div>
            <div>
              <label className="form-label">الوحدة</label>
              <input className="glass-input" value={wasteEntry.unit} readOnly style={{ background: 'rgba(248, 250, 252, 0.8)' }} />
            </div>
          </div>

          <label className="form-label"><AlertTriangle size={16} color="#e74c3c" /> سبب الهالك</label>
          <select className="glass-input" value={wasteEntry.reason} onChange={(e) => setWasteEntry({ ...wasteEntry, reason: e.target.value })} style={{ marginBottom: '15px' }}>
          ...
          <button type="submit" className="btn-primary" style={{ backgroundColor: '#e74c3c', boxShadow: '0 4px 15px rgba(231, 76, 60, 0.3)' }}>
