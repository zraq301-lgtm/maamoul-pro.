...
const Sales = ({ onBack, onSaveSale, customers = [], stock = [] }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!sale.customerName || !sale.quantity || !sale.pricePerUnit || !sale.productName) {
      alert("يرجى إكمال جميع بيانات البيع");
      return;
    }
    if (parseFloat(sale.quantity) > availableQty) {
      alert(`الكمية المطلوبة (${sale.quantity}) تتجاوز الرصيد المتوفر (${availableQty})`);
      return;
    }
    ...
    <div style={{ padding: '15px', direction: 'rtl', fontFamily: "'Tajawal', sans-serif", minHeight: '100vh' }}>
      ...
      <form onSubmit={handleSubmit}>
        <div className="glass-card" style={{ marginBottom: '15px' }}>
          ...
          <div style={{ marginBottom: '15px' }}>
            ...
            >
              ...
              {availableProducts.map(s => (
                <option key={s.id} value={s.name}>{s.name} (متوفر: {s.balance})</option>
              ))}
            </select>
          </div>
          ...
          {sale.productName && (
            <div style={{
              background: availableQty > 0 ? 'rgba(236, 253, 245, 0.8)' : 'rgba(254, 226, 226, 0.8)',
              padding: '10px 14px',
              borderRadius: '12px',
              marginBottom: '15px',
              display: 'flex',
              ...
              <AlertCircle size={16} />
              الرصيد المتوفر: {availableQty} {selectedProduct?.unit || 'وحدة'}
            </div>
          )}

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div>
              <label className="form-label"><Hash size={16} color="#2ecc71" /> الكمية</label>
              <input
                type="number"
                className="glass-input"
            ...
            <div>
              <label className="form-label"><DollarSign size={16} color="#2ecc71" /> السعر</label>
              <input
                type="number"
                className="glass-input"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button type="submit" className="btn-primary" style={{ backgroundColor: '#2ecc71', boxShadow: '0 4px 15px rgba(46, 204, 113, 0.3)' }}>
