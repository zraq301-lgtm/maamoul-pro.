...
const Suppliers = ({ onBack, onAddSupplier, suppliers = [], waitingList = [], onPayDebt, onUpdateWaitingList }) => {
  ...
  const handleRemoveOrder = (orderId) => {
    if (onUpdateWaitingList) {
      onUpdateWaitingList(waitingList.filter(o => o.id !== orderId));
    }
  };
  ...
  return (
    <div style={{ padding: '15px', direction: 'rtl', fontFamily: "'Tajawal', sans-serif", minHeight: '100vh' }}>
      ...
      <button
        ...
        style={{ backgroundColor: '#34495e', marginBottom: '15px', boxShadow: '0 4px 15px rgba(52, 73, 94, 0.3)' }}
      >
      ...
      {showAdd && (
        <div className="glass-card" style={{ marginBottom: '20px' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <label className="form-label"><DollarSign size={14} /> المديونية السابقة (إن وجدت)</label>
            ...
            <button type="submit" className="btn-primary" style={{ backgroundColor: '#34495e' }}>
              <Save size={18} /> حفظ المورد
            </button>
          </form>
        </div>
      )}

      {/* Suppliers List with Debt Management */}
        ...
        suppliers.map(s => (
          <div key={s.id} className="glass-card" style={{ marginBottom: '10px', padding: '15px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '8px' }}>
              ...
              {(s.debt || 0) > 0 && (
                <span className="status-badge" style={{ background: '#fee2e2', color: '#ef4444' }}>
            ...
            {(s.debt || 0) > 0 && onPayDebt && (
              <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                <input
                  ...
                  value={payAmount[s.name] || ''}
                  onChange={(e) => setPayAmount(prev => ({ ...prev, [s.name]: e.target.value }))}
                  style={{ marginBottom: 0, flex: 1 }}
                />
                <button
                  onClick={() => handlePayDebt(s.name)}
                  style={{
                    padding: '12px 16px',
                    borderRadius: '14px',
                    border: 'none',
            ...
            <div key={order.id} className="glass-card" style={{ marginBottom: '8px', padding: '12px', borderRight: '4px solid #f59e0b' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{ background: 'none', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '6px 10px', color: '#ef4444', cursor: 'pointer', fontSize: '0.8rem' }}
                >
                  حذف
                </button>
              </div>
